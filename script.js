// ===== Canvas ambience: sakura + snowflakes + gold sparks =====
...
function drawFlake(f, t){
  const sway = Math.sin(t*0.001 + f.swayPhase) * 0.5;
  ctx.save();
  ctx.translate(f.x, f.y);
  ctx.rotate(f.rot);
  ctx.globalAlpha = 0.9;
  // simple six-point flake
...
function tick(t){
  ...
  // petals
  if(t - last > 40 && petals.length < 110){ petals.push(makePetal()); last = t; }
  ...
  // snowflakes
  if(Math.random() < 0.25 && flakes.length < 140){ flakes.push(makeFlake()); }
...
if (bgm && musicBtn) {
  bgm.volume = 0.25;
  const setBtn = () => musicBtn.textContent = bgm.paused
    ? 'Play Xinzhi’s OST — “listen to his masterpiece chuchu”'
    : 'Pause Xinzhi’s OST';
